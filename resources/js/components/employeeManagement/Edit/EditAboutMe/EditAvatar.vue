...
export default {
    props: ["name", "errorMessage", "rules", "errorLabel", "url", "employeeId"],
    data() {
        return {
            value: File,
        };
    ...
    methods: {
            ...
            if (e.target.files.length != 0) {
                ...
                if (
                    ...
                    if (this.toKiloBytes(e.target.files[0].size) <= 2048) {
                        ...
                        reader.onload = (e) => {
                            EventBus.$emit("set-profile-picture-preview", e.target.result, this.employeeId);
                        };
                    } else {
        ...
        updateAvatar() {
            ...
            axios
                .post(`/api/employee/${this.employeeId}/update-avatar`, data, config)
                .then((response) => {
