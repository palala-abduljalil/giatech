<template>
    <v-app>
        <v-card>
            <v-card-title>
                <div class="align-end">
                    <v-btn color="primary" @click="handleCreateUser">
                        <v-icon left>mdi-plus-box</v-icon>
                        Create User
                    </v-btn>
                </div>
                <v-spacer></v-spacer>
                <v-text-field
                    v-model="search"
                    append-icon="mdi-magnify"
                    label="Search"
                    single-line
                    hide-details
                ></v-text-field>
            </v-card-title>
            <v-data-table
                :headers="headers"
                :items="items"
                :search="search"
                class="elevation-1"
            >
                <template v-slot:item.employee.name="{ item }">
                    <div class="user-block">
                        <img
                            class="img-circle"
                            :src="`${item.employee.profilePicture.url}`"
                            alt="User Image"
                        />
                        <span class="name">{{ item.employee.name }}</span>
                        <span class="position">{{
                            item.employee.position.name
                        }}</span>
                    </div>
                </template>
                <template v-slot:item.createdBy.name="{ item }">
                    <div class="user-block" v-if="item.createdBy">
                        <img
                            class="img-circle"
                            :src="`${item.createdBy.profilePicture.url}`"
                            alt="User Image"
                        />
                        <span class="name">{{ item.createdBy.name }}</span>
                        <span class="position">{{ item.createdBy.position.name }}</span>
                    </div>
                    <v-chip v-else color="blue" outlined>System Administrator</v-chip>
                </template>
                <template v-slot:item.isRenewed="{ item }">
                    <v-chip
                        v-if="item.isRenewed == 1"
                        class="ma-2"
                        color="green"
                        text-color="white"
                    >
                        Renewed
                    </v-chip>
                    <v-chip v-else class="ma-2" color="red" text-color="white">
                        Not Renewed
                    </v-chip>
                </template>
                <template v-slot:item.isActivated="{ item }">
                    <v-chip
                        v-if="item.isActivated == 1"
                        class="ma-2"
                        color="green"
                        text-color="white"
                    >
                        Activated
                    </v-chip>
                    <v-chip v-else class="ma-2" color="red" text-color="white">
                        Not Activated
                    </v-chip>
                </template>
                <template v-slot:item.status.description="{ item }">
                    <v-chip
                        v-if="item.status.id == 1"
                        class="ma-2"
                        color="green"
                        text-color="white"
                    >
                        Online
                    </v-chip>
                    <v-chip v-else class="ma-2" color="red" text-color="white">
                        Offline
                    </v-chip>
                </template>
                <template v-slot:item.action="{ item }">
                    <v-menu bottom left>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn icon v-bind="attrs" v-on="on">
                                <v-icon>mdi-dots-vertical</v-icon>
                            </v-btn>
                        </template>
                        <v-list>
                            <v-list-item link @click="handleView(item)">
                                <v-list-item-title>
                                    <v-icon small left> mdi-eye </v-icon>
                                    View
                                </v-list-item-title>
                            </v-list-item>
                            <v-list-item link @click="handleEdit(item)">
                                <v-list-item-title>
                                    <v-icon small left> mdi-pencil </v-icon>
                                    Edit
                                </v-list-item-title>
                            </v-list-item>
                            <v-list-item
                                link
                                @click="handleRemove(item)"
                                v-if="$gate.getUserID() != item.id"
                            >
                                <v-list-item-title>
                                    <v-icon small left> mdi-delete </v-icon>
                                    Remove
                                </v-list-item-title>
                            </v-list-item>
                            <v-list-item link @click="handleResetPassword(item)">
                                <v-list-item-title>
                                    <v-icon small left> fas fa-redo </v-icon>
                                    Reset Password
                                </v-list-item-title>
                            </v-list-item>
                            <v-list-item
                                v-if="item.isActivated == 0"
                                link
                                @click="handleActivate(item)"
                            >
                                <v-list-item-title>
                                    <v-icon small left>
                                        fas fa-toggle-on
                                    </v-icon>
                                    Activate
                                </v-list-item-title>
                            </v-list-item>
                            <v-list-item v-else link @click="handleDeactivate(item)">
                                <v-list-item-title>
                                    <v-icon small left>
                                        fas fa-toggle-off
                                    </v-icon>
                                    Deactivate
                                </v-list-item-title>
                            </v-list-item>
                        </v-list>
                    </v-menu>
                </template>
            </v-data-table>
        </v-card>
    </v-app>
</template>

<script>
export default {
    props: {
        items: Array,
        headers: Array
    },
    data() {
        return {
            search: "",
        };
    },
    methods: {
        handleCreateUser() {
            this.$emit("onCreateUser");
        },
        handleEdit(item) {
            this.$emit("onEdit", item);
        },
        handleView(item) {
            this.$emit("onView", item);
        },
        handleRemove(item) {
            this.$emit("onRemove", item);
        },
        handleResetPassword(item) {
            this.$emit("onResetPassword", item);
        },
        handleActivate(item) {
            this.$emit("onActivate", item);
        },
        handleDeactivate(item) {
            this.$emit("onDeactivate", item);
        },
    }
};
</script>

<style lang="scss" scoped>
.user-block {
    float: left;

    img {
        float: left;
        height: 40px;
        width: 40px;
    }

    .name,
    .position {
        display: block;
        margin-left: 50px;
    }

    .name {
        font-size: 16px;
        margin-top: -1px;
    }

    .position {
        font-size: 13px;
        margin-top: -3px;
    }
}
</style>
